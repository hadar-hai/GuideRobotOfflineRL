...
dataset_path = "./data/processed_data/game_and_map_data/"
saving_path_videos = "./postprocessing/replaying_videos/"
saving_path_images = "./data/processed_data/processed_data_images/"
video_images_both_flag = "video"  # "both" or "video" or "images"
...
# create function 
def create_image_and_video_from_data_file(dataset_path, filename, video_images_both_flag, WIDTH=800, HEIGHT=600):
        # Load the data
    data = pd.read_csv(dataset_path + filename + ".csv")
    if video_images_both_flag == "video" or video_images_both_flag == "both":
        if saving_path_videos.split('/')[-2] not in os.listdir('./postprocessing/'):
            os.mkdir(saving_path_videos)
        if saving_path_images.split('/')[-2] not in os.listdir('./data/processed_data/'):
...
if create_one_file:
    filename = 'dataset_1_game_02'    
    create_image_and_video_from_data_file(dataset_path, filename, video_images_both_flag)
else:
