...
scaler_path = r".\data_based_agents\models\scalers\scaler_pytorch_without_not_moving_less_beams.pkl"
...
# create features_ data structure like features 
features_ = pd.DataFrame()

for i in range(0, 360, 10):
    id = get_title(i)
    # keep only this columns
    features_ = features_._append(features[[f'beam_{id}_dist_to_obj1', f'beam_{id}_dist_to_obj2', f'beam_{id}_dist_to_obj3']])
...
plt.ylabel('Loss')
plt.legend()
plt.savefig(r".\data_based_agents\plots\train_val_loss_lidar_pytorch_without_not_moving_less_beams.png")
plt.close()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig(r".\data_based_agents\plots\val_accuracy_lidar_pytorch_without_not_moving_less_beams.png")
...
plt.savefig(r".\data_based_agents\plots\confusion_matrix_lidar_pytorch_without_not_moving_less_beams.png")

# Save the scaler
joblib.dump(scaler, scaler_path)

#save the model 
torch.save(model.state_dict(), r".\data_based_agents\models\behavior_cloning_lidar_pytorch_without_not_moving_less_beams.pth")
